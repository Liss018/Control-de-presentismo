...
if (empty($turno) || empty($id_ano) || empty($id_division)) {
    die("Faltan parámetros necesarios para la consulta.");
}

// Obtener la asistencia filtrada por turno, año y división
$query = "SELECT ra.nombre, ra.apellido, a.asistencia, a.observaciones
          ...
          WHERE a.fecha = ? AND d.turno = ? AND ra.id_ano = ? AND d.Nr = ?";

$stmt = $conexion->prepare($query);
$stmt->bind_param("ssss", $fecha, $turno, $id_ano, $id_division);
$stmt->execute();
...
<header class="header">
    ...
    <div class="container">
        ...
        <div class="navigation">
            <a href="ver_asistencia.php?fecha=<?php echo urlencode($fecha_anterior); ?>&turno=<?php echo urlencode($turno); ?>&id_ano=<?php echo urlencode($id_ano); ?>&id_division=<?php echo urlencode($id_division); ?>" class="arrow left">←</a>
            <a href="ver_asistencia.php?fecha=<?php echo urlencode($fecha_posterior); ?>&turno=<?php echo urlencode($turno); ?>&id_ano=<?php echo urlencode($id_ano); ?>&id_division=<?php echo urlencode($id_division); ?>" class="arrow right">→</a>
        </div>
...
<a class="button" href="Asistencia_admi.php?turno=<?php echo urlencode($turno); ?>&año=<?php echo urlencode($id_ano); ?>&id_division=<?php echo urlencode($id_division); ?>" >Volver</a>
</div>
    <footer>
