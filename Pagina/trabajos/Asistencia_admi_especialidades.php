...
$id_ano = isset($_GET['id_ano']) ? intval($_GET['id_ano']) : 0; // Cambiado a id_ano
$id_division = isset($_GET['id_division']) ? intval($_GET['id_division']) : 0; // Añadido para obtener la división
$especialidad = isset($_GET['especialidad']) ? htmlspecialchars($_GET['especialidad']) : ''; // Obtener especialidad

// Consulta para obtener información de la división y el año
$sql_division = "SELECT division.nr AS curso, division.turno AS turno, año.descripcion AS año
                 FROM division
                 INNER JOIN año ON division.año = año.id_ano
                 WHERE division.id = $id_division";
$result_division = $conexion->query($sql_division);
...
<header class="header">
    ...
    <hr>
    <button class="volver-boton" onclick="window.location.href='divisiones_especialidad.php?especialidad=<?php echo urlencode($especialidad); ?>&turno=<?php echo urlencode($turno); ?>&id_ano=<?php echo $id_ano; ?>';">Volver</button>

</header>

<h2>Alumnos del año <?php echo htmlspecialchars($row_division['año']); ?>, curso <?php echo htmlspecialchars($row_division['curso']); ?> y especialidad <?php echo htmlspecialchars($row_division['turno']); ?></h2>
...
<button class="vol" onclick="window.location.href='ver_asistencia_admin.php?turno=<?php echo urlencode($turno); ?>&id_ano=<?php echo $id_ano; ?>&id_division=<?php echo $id_division; ?>&especialidad=<?php echo urlencode($especialidad); ?>';">
    Ver Asistencia
</button>
